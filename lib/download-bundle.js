    ...
    request(getOptions(config))
        .on('response', response => {
            if (response.statusCode !== 200) {
                ...
                cleanDir(zipOutputFile)
            } else {
                log(colors.info('Status: %s\n'), response.statusCode)
            }
        })
        .on('error', err => {
            console.log("Error: asd", err)
        })
        .pipe(fs.createWriteStream(zipOutputFile))
        .on('close', () => {

            var zip = new AdmZip(zipOutputFile);
            zip.extractAllTo(api_name, /*overwrite*/true);
